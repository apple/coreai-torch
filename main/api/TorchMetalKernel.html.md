...
For a tutorial walkthrough, see [Custom Metal Kernels](../guides/custom-metal-kernels.md).
...
| `torch_defn`      | `Callable`                    | —         | Reference PyTorch implementation used for shape inference during `torch.export`. See [Constraints]() below.                                                                                |
...
See [TorchConverter API reference](TorchConverter.md) for `register_custom_kernels` details.
